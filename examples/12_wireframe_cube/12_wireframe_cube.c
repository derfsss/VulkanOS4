/*
** Vulkan Wireframe Cube -- AmigaOS 4
**
** Demonstrates matrix push constants: a rotating wireframe
** cube rendered with LINE_LIST topology. The MVP matrix is computed on
** the CPU each frame and passed to the vertex shader via push constants.
**
** Also demonstrates:
** - Vertex buffers (vec3 positions)
** - Index buffers (16-bit indices, LINE_LIST edges)
** - mat4 push constants (rotation + perspective projection)
**
** Compile: ppc-amigaos-gcc -mcrt=newlib -D__USE_INLINE__
**          -o wireframe_cube wireframe_cube.c -lvulkan_loader -lauto -lm
*/
...
int    vkex_dur = vkex_duration_secs(argc, argv);    time_t vkex_t0  = time(NULL);
    ...
    printf("=== Vulkan Wireframe Cube ===\n");
    printf("Green wireframe cube rotating in 3D with perspective projection.\n\n");

    if (!IVulkan) { printf("ERROR: No vulkan.library\n"); return 1; }

    /* --- Cube geometry --- */
    float cubeVerts[] = {
        /* 8 vertices: vec3 position */
    ...
    uint16_t cubeIndices[] = {
        /* 12 edges x 2 indices = 24, LINE_LIST topology */
    ...
    ai.pApplicationName = "Wireframe Cube";
    ...
    window = OpenWindowTags(NULL,
        WA_Title, (Tag)"Vulkan Wireframe Cube",
    ...
    pcRange.offset = 0;
    pcRange.size = 64; /* sizeof(mat4) = 16 floats * 4 bytes */
    ...
    /* Vertex input: single binding, vec3 position */
    VkVertexInputBindingDescription vibd;
    memset(&vibd, 0, sizeof(vibd));
    vibd.binding = 0;
    vibd.stride = 3 * sizeof(float); /* vec3 */
    ...
    printf("Wireframe cube... close window to exit.\n");
    ...
    while (running)
    {
        ...
        vkm_mat4_rotate_x(rotX, 0.5f); /* slight tilt for 3D appearance */
        ...
        /* Move cube back so it's visible (translate Z by -3) */
        float view[16];
        ...
        /* Bind vertex + index buffers, draw indexed lines */
        ...
        vkCmdDrawIndexed(cmdBuf, 24, 1, 0, 0, 0); /* 24 indices, 12 edges */
