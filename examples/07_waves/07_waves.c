/*
** Vulkan Waves -- AmigaOS 4
**
** Renders a fullscreen quad using TRIANGLE_STRIP topology
** (4 vertices, 2 triangles) with animated wave color patterns.
**
** Demonstrates OpExtInst Sin/Cos for animated wave color patterns.
** Uses push constants to pass time offset to the fragment shader.
** This is a performance stress test -- sin/cos runs on every pixel.
**
** Compile: ppc-amigaos-gcc -mcrt=newlib -D__USE_INLINE__
**          -o waves waves.c -lvulkan_loader -lauto
*/
...
int main(int argc, char **argv)
{
    ...
    ai.pApplicationName = "Vulkan Waves";
    ...
    /* Pipeline layout with push constants for time */
    ...
    printf("Animated waves... close window to exit.\n");

    /* Animated render loop with push constant time */
    float time = 0.0f;
    uint32_t frame = 0;
    ...
    while (running)
    {
        ...
        /* Push time to fragment shader */
        vkCmdPushConstants(cmdBuf, pipLayout, VK_SHADER_STAGE_FRAGMENT_BIT,
                           0, sizeof(float), &time);
